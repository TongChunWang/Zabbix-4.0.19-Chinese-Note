...
/******************************************************************************
 * *
 *整个代码块的主要目的是从进程ID为pid的进程中获取进程参数（argc和argv），并将获取到的argc和argv存储在传入的指针变量中。输出结果为成功（SUCCEED）或失败（FAIL）。在获取进程参数过程中，如果内存不足，会自动扩大argv的分配大小并重新分配内存。
 ******************************************************************************/
static int	proc_argv(pid_t pid, char ***argv, size_t *argv_alloc, int *argc)
{
	// 定义一个长度为4的整型数组mib，用于存储系统调用所需的参数
	size_t	sz;
	int	mib[4];

	// 如果argc指针为空，则初始化argc分配大小为ARGS_START_SIZE，并分配内存空间给argv
	if (NULL == *argv)
	{
	...
	// 初始化mib数组，第一个元素为CTL_KERN，表示控制内核；第二个元素为KERN_PROC_ARGS，表示获取进程参数
	mib[0] = CTL_KERN;
	mib[1] = KERN_PROC_ARGS;
	mib[2] = (int)pid; // 第三个元素为进程ID
	mib[3] = KERN_PROC_ARGV; // 第四个元素为进程参数
retry:
	// 尝试获取进程参数，并将分配大小记录在sz变量中
	sz = *argv_alloc;
	if (0 != sysctl(mib, 4, *argv, &sz, NULL, 0))
	{
		// 如果获取进程参数失败，且错误原因是内存不足（ENOMEM），则扩大argc的分配大小并重新分配内存
		if (errno == ENOMEM)
		{
			*argv_alloc *= 2;
