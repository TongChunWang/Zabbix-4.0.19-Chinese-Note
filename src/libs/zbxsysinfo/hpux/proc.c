...
/******************************************************************************
 * *
 *整个代码块的主要目的是检查zbx_proc_stat与pst的pst_stat之间的关系，根据不同的zbx_proc_stat值返回不同的结果。如果zbx_proc_stat等于ZBX_PROC_STAT_ALL，则返回成功。如果zbx_proc_stat等于ZBX_PROC_STAT_RUN、ZBX_PROC_STAT_SLEEP或ZBX_PROC_STAT_ZOMB，则根据pst的pst_stat值判断是否匹配，匹配则返回成功，否则返回失败。如果zbx_proc_stat不属于以上三种情况，则默认返回失败。
 ******************************************************************************/
// 定义一个静态函数check_procstate，接收两个参数：一个struct pst_status类型的指针pst和一个整型变量zbx_proc_stat
static int	check_procstate(struct pst_status pst, int zbx_proc_stat)
{
	// 如果zbx_proc_stat等于ZBX_PROC_STAT_ALL，则返回成功
	if (ZBX_PROC_STAT_ALL == zbx_proc_stat)
		return SUCCEED;

	// 使用switch语句根据zbx_proc_stat的值进行分支处理
	switch (zbx_proc_stat)
	{
		// 如果zbx_proc_stat等于ZBX_PROC_STAT_RUN，则判断pst的pst_stat是否等于PS_RUN，如果是，则返回成功，否则返回失败
