...
/******************************************************************************
 * *
 *整个代码块的主要目的是实现一个名为tcp_expect的函数，该函数用于预期TCP连接的行为。它接受以下参数：
 *
 *1. `host`：远程服务器的IP地址或域名。
 *2. `port`：远程服务器的端口号。
 *3. `timeout`：连接超时时间。
 *4. `request`：发送到远程服务器的请求字符串。
 *5. `validate_func`：验证函数，用于对接收到的数据进行验证。
 *6. `sendtoclose`：发送给远程服务器以关闭连接的字符串。
 *7. `value_int`：指向一个整型变量的指针，用于存储预期结果。
 *
 *函数执行以下操作：
 *
 *1. 尝试连接到远程服务器。
 *2. 如果请求字符串不为空，发送请求到远程服务器。
 *3. 如果验证函数不为空，对接收到的数据进行验证。
 *4. 如果关闭套接字的字符串不为空，发送关闭请求。
 *5. 关闭套接字。
 *6. 如果连接成功且验证成功，将value_int指向的整型变量设为1。
 *7. 记录日志并返回操作结果。
 ******************************************************************************/
               ...
               int (*validate_func)(const char *), const char *sendtoclose, int *value_int)
{
    ...
    zbx_socket_t	s;
    // 定义一个指向字符串的指针变量buf，用于存储接收到的数据
    const char	*buf;
    // 定义一个整型变量net，用于存储套接字连接状态
    int		net, val = ZBX_TCP_EXPECT_OK;

    // 初始化value_int指向的整型变量，将其值设为0
    *value_int = 0;

    // 尝试连接到远程服务器，如果连接成功，将net设置为ZBX_TCP_EXPECT_OK，否则退出函数
    if (SUCCEED != (net = zbx_tcp_connect(&s, CONFIG_SOURCE_IP, host, port, timeout, ZBX_TCP_SEC_UNENCRYPTED, NULL,
                                           NULL)))
    {
        goto out; // 如果连接失败，跳转到out标签处
    }

    // 如果请求字符串不为空，发送请求到远程服务器
    if (NULL != request)
        net = zbx_tcp_send_raw(&s, request);

    // 如果验证函数不为空且连接成功，执行验证操作
...
/******************************************************************************
 * *
 *整个代码块的主要目的是：定义一个名为 NET_DNS 的函数，用于处理 DNS 查询请求。传入的两个参数分别是请求信息和结果集，通过调用 dns_query 函数来实现 DNS 查询功能。返回值可能是查询结果或其他相关信息。
 ******************************************************************************/
// 定义一个名为 NET_DNS 的函数，接收两个参数，分别是 AGENT_REQUEST 类型的 request 和 AGENT_RESULT 类型的 result。
int NET_DNS(AGENT_REQUEST *request, AGENT_RESULT *result)
{
    // 调用 dns_query 函数，传入三个参数：request、result 和 1。
    // 这里的 1 可能是用于表示 DNS 查询类型，如 A、MX 等。
    return dns_query(request, result, 1);
}

/******************************************************************************
 * *
 *整个代码块的主要目的是实现一个名为 NET_DNS_RECORD 的函数，该函数用于处理 DNS 查询请求。函数接收两个参数，分别是 AGENT_REQUEST 类型的 request 和 AGENT_RESULT 类型的 result。函数内部调用 dns_query 函数，传入三个参数：request、result 和 0。函数返回值为 dns_query 的返回值。
 ******************************************************************************/
// 定义一个名为 NET_DNS_RECORD 的函数，接收两个参数，分别是 AGENT_REQUEST 类型的 request 和 AGENT_RESULT 类型的 result。
int NET_DNS_RECORD(AGENT_REQUEST *request, AGENT_RESULT *result)
{
    // 调用 dns_query 函数，传入三个参数：request、result 和 0。
    // 函数返回值为 dns_query 的返回值。
    return dns_query(request, result, 0);
}

// 整个代码块的主要目的是实现一个名为 NET_DNS_RECORD 的函数，该函数用于处理 DNS 查询请求。
